...
public class SalesControlFlowPIDWithFixedInventoryTest {
    ...
    public void activatesCorrectly()
    {
        ...
        for(int i=1; i <10; i++)
        {
            ...
            pricing.step(state);
            Assert.assertEquals(pricing.getTarget(),0); //because you are building up
        }
        ...
        pricing.step(state);
        Assert.assertEquals(pricing.getTarget(),5); //because you are done building up
        Assert.assertEquals(pricing.getPhase(), SalesControlFlowPIDWithFixedInventory.SimpleInventoryAndFlowPIDPhase.SELL);

        //inventory goes down, but we should still be in sell phase
        when(department.getTodayInflow()).thenReturn(5);
        when(department.getHowManyToSell()).thenReturn(25);
        pricing.step(state);
        Assert.assertEquals(pricing.getTarget(),5); //still you should do your job, even though you are below that limit
        Assert.assertEquals(pricing.getPhase(), SalesControlFlowPIDWithFixedInventory.SimpleInventoryAndFlowPIDPhase.SELL);
        ...
        pricing.step(state);
        Assert.assertEquals(pricing.getTarget(),0); //back to buildup!
        Assert.assertEquals(pricing.getPhase(), SalesControlFlowPIDWithFixedInventory.SimpleInventoryAndFlowPIDPhase.BUILDUP);
    ...
    public void priceChanges()
    {
        ...
        for(int i=1; i <10; i++)
        {
            ...
            when(department.getHowManyToSell()).thenReturn(i * 5);
            pricing.step(state);
            Assert.assertEquals(pricing.getPrice(),100);
        }
        ...
        when(department.getHowManyToSell()).thenReturn(50);
        pricing.step(state);
        Assert.assertTrue(Long.toString(pricing.getPrice()),pricing.getPrice() < 100); //price should have gone down!!!!
        ...
        when(department.getHowManyToSell()).thenReturn(25);
        pricing.step(state);
        Assert.assertTrue(pricing.getPrice() < newPrice); //price should have gone down further
        newPrice = pricing.getPrice();

        //same target, but now today outflow is high!
        when(department.getTodayOutflow()).thenReturn(20);
        pricing.step(state);
        Assert.assertTrue(pricing.getPrice()>newPrice); //price should have gone up!
        newPrice = pricing.getPrice();
...
//        Assert.assertEquals(pricing.getPrice(),newPrice); //price is back to be stuck
