...
public class StandardPlantCostStrategyTest {
    ...
    public void initialTest(){
        ...
        Assert.assertEquals(strategy.getTotalProductionPerRun(),4);
        Assert.assertEquals(strategy.getWageCostsPerUnit(),0); //there is no worker
        Assert.assertEquals(strategy.unitOutputCost(GoodType.CAPITAL, 400),100);


        boolean exceptionThrown = false;
        try{
            Assert.assertEquals(strategy.unitOutputCost(GoodType.GENERIC, 400),0);
        ...
        //get wages paid
        Assert.assertEquals(strategy.getWageCostsPerUnit(),Math.round(3f*3f/4f)); // 3 workers paid 3, divided by 4 (weekly production rate)

        when(p.hypotheticalTotalThroughput(anyInt())).thenReturn(8f);
        strategy.changeInWorkforceEvent(p,3);
        Assert.assertEquals(strategy.getWageCostsPerUnit(), Math.round(3f * 3f / 8f)); // 3 workers paid 3, divided by 8 (weekly production rate)
        ...
        Assert.assertEquals(strategy.getWageCostsPerUnit(),Math.round(3f*3f/12f)); // 3 workers paid 3, divided by 12 (weekly production rate : 4+2 produced twice)
    ...
    public void BetterDressedTest(){
        ...
        strategy.changeInWorkforceEvent(p,3);
        Assert.assertEquals(strategy.getWageCostsPerUnit(),Math.round(3f*3f/8f)); // 3 workers paid 3, divided by 4 (weekly production rate)
