...
/** A wrapper for SimState and Schedule which provides additional functionality for
    ...
    <p>GUIState has a wrapper adjust() method which in turn calls the Schedule's
    adjust(state) method.  However, this wrapper lso provides a hook for objects such as
    ...
    start(), finish(), adjust(), reset(), and readNewStateFromCheckpoint()
...
public abstract class GUIState
    ...
    */
    ...
    /** Schedules an item to occur (in no particular order) immediately before 
        the schedule is stepped on the next time adjust (not including blank steps).
        Pass in FALSE to indicate you want to be immediately BEFORE the next timestep;
        pass in TRUE if you want to be immediately AFTER the next time adjust (the more common
