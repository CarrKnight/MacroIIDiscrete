...
public class Console extends JFrame implements Controller
    ...
    /** Default maximum number of steps in the adjust slider */
    ...
    JLabel sliderText;
    /** The slider which controls the number of steps per press of the adjust-button */
    JSlider stepSlider;
    /** The associated text for number of steps per press of the adjust-button */
    ...
    Vector frameList;
    /** Where the user can enter in a adjust count to stop at */
    PropertyField endField;
    /** Where the user can enter in a adjust count to pause at */
    ...
    /** how many steps we should take on one press of the "adjust" button.  As this is only relevant
    ...
    public Console(final GUIState simulation)
        ...
        */
        ...
        // Create the adjust slider
        // the equation is: adjust = slider
...
//            prioritySlider.setValue(appPrefs.getInt(THREAD_PRIORITY_KEY, systemPrefs.getInt(THREAD_PRIORITY_KEY, prioritySlider.getValue())));
    ...
    */
    ...
    /** Milliseconds of how long we should sleep between each adjust. Don't play with this. */
    long playSleep = 0;
    
    /** Sets (in milliseconds) how long we should sleep between each adjust in the play thread.
    ...
    /** Gets how long we should sleep between each adjust in the play thread (in milliseconds). */
    ...
    void startSimulation()
        ...
        if (modelInspector != null)
            ...
            // stepper.adjust(simulation.state);  // update the model inspector one time at the beginning at any rate
            }
        }
...
//            setRandomNumberGenerator(randomSeed);
    ...
    /** Called when the user presses the play button.  You can call this as well to simulate the same.  Keep in mind that play will change to adjust if pause is down. */
    public synchronized void pressPlay()
        ...
        else if (getPlayState() == PS_PAUSED) // adjust N times
            {
