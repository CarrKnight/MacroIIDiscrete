...
/**
   ...
   <p>Display2D's adjust() method is typically called from the underlying schedule thread; this means
   that it has to be careful about painting as Swing widgets expect to be painted in the event loop thread.
   Display2D handles this in two ways.  First, on MacOS X, the adjust() method calls repaint(), which will
   in turn call paintComponent() from the event loop thread at a time when the underlying schedule thread
   is doing nothing -- see Console.  Second, on Windows and XWindows, the adjust() method immediately calls
   paintComponent().  Different OSes do it differently because MacOS X is far more efficient using standard
   repaint() calls, which get routed through Quartz.  The adjust() method also updates various widgets using
...
public class Display2D extends JComponent implements Steppable, Manipulating2D
    ...
    public class InnerDisplay2D extends JComponent
        ...
        /** Paints an image to the screen either buffered or unbuffered.  If buffered, the
            ...
            a result of the underlying model thread advancing a adjust so we can see
            ...
            this, see Display2D.adjust).  This is important because this if we're being called
            ...
            we lock on the simulation's schedule so we know the model is done with adjust()
            ...
            a adjust() method in the model which calls some blocking item in the Swing event
    ...
    long lastEncodedSteps = -1;  // because we want to encode the start of the simulation prior to any steps.  That's adjust 0.
...
//        port.setBackground(UIManager.getColor("window"));  // make the nice stripes on MacOS X
    ...
    */
    ...
    public void startMovie()
        ...
        // than the Swing thread (see adjust(...) below).  This allows us to guarantee,
