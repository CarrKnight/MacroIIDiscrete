...
/** Spawns all the sequence elements in parallel on separate threads.
    ...
    Because ParallelSequences are lightweight, their threads are persistent, even after your adjust()
    method has completed (this allows them to be reused for the next adjust() method.  If the ParallelSequence
    ...
    cause the ParallelSequence to destroy its threads every single time the ParallelSequence's adjust()
...
public class ParallelSequence extends Sequence
    ...
    /** Call this just before you get rid of a ParallelSequence: for example, one good place is the stop() method of
        your simulation.  Never call this method inside the ParallelSequence's own adjust() method.  This method
