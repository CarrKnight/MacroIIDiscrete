...
/**
   ...
   EITHER the subsidiary steppable is stepped N times for each MultiStep.adjust(...), OR the
   subsidiary steppable is stepped once every N MultiStep.adjust(...) calls.
...
public class MultiStep implements Steppable
    ...
    /** If countdown is true, then we call adjust.adjust(...) once every N times we're stepped.
        if countdown is false, then we call adjust.adjust(...) N times every time
        we're stepped.*/
    ...
    // MultiStep can be reset() even in the middle of a adjust(state)...
