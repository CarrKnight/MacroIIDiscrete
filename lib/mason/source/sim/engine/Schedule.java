...
/**
   ...
   <p>The schedule is pulsed by calling its <b>adjust(...)</b> method.  Each pulse, the schedule
   ...
   in the queue.  Stopping the simulation from within a Steppable object's adjust() method is best done by
   calling SimState.kill().  From the main thread, the most straightforward way to stop a simulation is to just
   stop calling schedule.adjust(...), and proceed directly to SimState.finish().
   
   <p>You can get the number of times that adjust(...) has been called on the schedule by calling the getSteps() method.
   This value is incremented just as the Schedule exits its adjust(...) method and only if the method returned true.
   ...
   Schedule uses two locks for synchronization.  First, the <b>adjust() method synchronizes on the Schedule</b> itself.  This
   prevents adjust() from being called simultaneously from different threads; also adjust() tests to make sure that it's not
   called reentrantly from within the same thread.  Second, <b>many methods synchronize on an internal lock</b>, including adjust().
   This allows adjust() to synchronize on the lock only to suck out the relevant Steppables from the Heap and to advance the timestep;
   all other portions of adjust() are outside of the lock.  Thus when adjust() actually steps the Steppables, even in different threads
   (like AsynchronousSteppable or ParallelSequence), they can turn around and submit adjust-requests to the Schedule even while it's still
   in its adjust() method.
   
   <p>One downside to this flexibility is that it's very inefficient to check, at each adjust of a Steppable, whether the Schedule
   has been reset or not.  Thus now if you call reset() or [better] SimState.kill(), the Schedule will continue to adjust Steppables
...
public class Schedule implements java.io.Serializable
    ...
    // the number of times adjust() has been called on me
    long steps;
    ...
    */
    ...
    /** Steps the schedule, gathering and ordering all the items to adjust on the next time adjust (skipping
    ...
    public synchronized boolean step(final SimState state)
        ...
        if (inStep)  // check for reentrant calls and deny
            {
            throw new RuntimeException("Schedule.adjust() is not reentrant, yet is being called recursively.");
            }
...
/**
   ...
   which is itself Steppable, and on its adjust calls its subsidiary Steppable, then reschedules
