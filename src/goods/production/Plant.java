...
/**
 * <h4>Description</h4>
 * <p/> This is the class representing a manufacturing/mining department able to produce a set of goods at a specific rate
 * <p/>
 * <p/>
 * <h4>Notes</h4>
...
public class Plant implements Agent, InventoryListener {
    ...
    /**
     * The technology used by the firm to produce whatever the blueprint says.
     */
    ...
    public void produce(){
        ...
        //DONE! now call the adjust again
        this.step(state);
     ...
     *      </ul></li>
     *
     *      <li>Draw a new wait time</li>
     *      <li>At the end of the wait-time, produce</li>
     *      <li>Produce will call the adjust, and on we go!</li>
     *  </ul>
     *  It returns if one of the two checks fails, at which point it has to be restarted manually
     * @param state the link to the Simstate
     */
    @Override
    public void step(SimState state) {
        ...
        //chec age
        if(status == PlantStatus.OBSOLETE) //if you are too old to produce
            return; //don't
        ...
        float waitTime = plantMachinery.nextWaitingTime(); //how much time will it take to produce?
        state.schedule.scheduleOnceIn(waitTime, new Steppable() {    //and after the wait time is over
            @Override
            public void step(SimState state) {
                //register production has ended!
                getOwner().logEvent(Plant.this, MarketEvents.PRODUCTION_COMPLETE, getOwner().getModel().getCurrentSimulationTimeInMillis());
                produce();     //BUILD the thing
            }
        });

        //produce will call this again
   ...
   * DELEGATE METHODS FROM THE TECHNOLOGY
    ...
    */
    ...
    /**
     * When the plant is not producing and a new input comes in, it checks if it's time to produce!
     *
    ...
    public void inventoryIncreaseEvent(@Nonnull HasInventory source, @Nonnull GoodType type, int quantity) {
        if(status == PlantStatus.PRODUCING || plantMachinery == null) //if you are busy or without machinery, don't bother
            return;

        Integer input =  blueprint.getInputs().get(type);
        if(input == null || input == 0 || quantity == 0) //don't bother working if it's not an input or there is none now
            return;
     ...
     *
     * @param outputType the specific good we want to know how much we produce of
     */
