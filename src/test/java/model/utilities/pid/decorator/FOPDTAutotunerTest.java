...
public class FOPDTAutotunerTest {

   // @Test
    public void prettyPictureTest() throws Exception {
        MacroII model = new MacroII(System.currentTimeMillis());
        final FOPDTAutotuner fopdtAutotuner = runLearningExperimentWithUnknownDeadTime(model.getRandom(), model.drawProportionalGain(), model.drawIntegrativeGain(), 0, 1.5f, 0.3f, 5, null);
        System.out.println(fopdtAutotuner.getDelay() + "-----" + fopdtAutotuner.getGain() + " ----" + fopdtAutotuner.getTimeConstant());
    }



    private FOPDTAutotuner runLearningExperimentWithUnknownDeadTime(MersenneTwisterFast random, float proportionalParameter,
        ...
        FOPDTAutotuner controller = new FOPDTAutotuner(new PIDController(proportionalParameter,integrativeParameter,0,random));
